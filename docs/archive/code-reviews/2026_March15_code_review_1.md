...
| # | 位置 | 問題 | 嚴重度 |
|---|------|------|--------|
| 1 | ~~`docker-compose.waf.yml:20`~~ | ~~WAF 設為 **DetectionOnly** 模式~~ — **已解決**：WAF 改由 Cloudflare WAF 處理，ModSecurity overlay 已移除 | ✅ Fixed |
...
| 6 | ~~`deploy/waf/REQUEST-900-EXCLUSION-RULES-BEFORE-CRS.conf`~~ | ~~WAF 排除規則過寬~~ — **已解決**：ModSecurity overlay 已移除，WAF 改由 Cloudflare WAF 處理 | ✅ Fixed |
