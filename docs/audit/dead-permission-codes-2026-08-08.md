...
後端有 **11 個權限碼被 `require_permission!` / `has_permission` 檢查，但不存在於 `permissions` 表**。
...
### 2-2 Group B — 已寫進角色授予清單，卻靜默沒生效（1 個）
...
- **Group B**：需要使用者裁定是否讓那五個角色真的取得 `aup.review.reply`。
...
`permissions.generated.ts` 與 DB 一致，管不到後端自己的裸字串——正是這 11 個漏網的原因。
...
- **PR 4**：`PartnerTable` 的刪除鈕刻意未上閘，等 `erp.partner.delete` 補進目錄後改成 `<Can>`。
