...
- **控制流扁平優於巢狀**：條件巢狀 ≤3 層，用 early return（`return Err(...)` 衛語句）、`?`、
  `let ... else` 把失敗路徑先擋掉，剩下的主體即「已知合法」的情況。
  例外：`match` 分支內的 `if let` 解 Option 屬慣用寫法，不必為降縮排而重構
  （2026-08-06 現況掃描：`?` 5,515 處 / 286 檔、衛語句 734 處，既有 code 已符合；
  本條是給**新程式碼**的標準，2026-08-06 CLAUDE.md 瘦身時連同「量化門檻」整段刪掉，此處補回）。
