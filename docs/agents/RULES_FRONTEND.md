...
- **控制流扁平優於巢狀**：條件巢狀 ≤3 層，用 early return 把 loading / 空資料 / 無權限
  等狀況先擋掉再渲染主體（`if (!data) return <Empty />`）；handler 與 hook 內用 guard clause
  取代長 if-else 鏈。2026-08-06 CLAUDE.md 瘦身時「量化門檻」整段被刪，此處補回。
...
- ⚠️ **junction 方向只能 worktree → 主 repo，絕不可反向**。反向（把主 repo 的套件指向
  ...
  **所有 session 都無法在本機跑 tsc/eslint**（2026-08-06 實測：主 repo 436 個頂層套件中 44 個
  junction 指向一個已被移除的 worktree，含 react / typescript / eslint / vite。
  註：此處刻意不寫該 worktree 名稱——守衛 hook 比對整條指令文字，寫死會讓未來 session
...
- 拆 junction 用 `[System.IO.Directory]::Delete('<path>', $false)`（非遞迴，只斷連結）。
  **不可用 `rm -r`**——會穿透 junction 把主 repo 的依賴刪掉。
  ...
  且 exit code 仍為 0），改用 `powershell.exe -NoProfile -Command`。
