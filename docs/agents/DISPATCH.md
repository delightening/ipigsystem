...
>
> **2026-08-03 ablation**：§1 指揮官不下場、§2 派工三件套、§4 回報合約、§6 升降級路徑、
> §8 主對話的自我保護 已移除（原文見 git 歷史，ablation 前的 `origin/main`；當日稍早 #1123 的
> 備份 `backup/DISPATCH.md.2026-08-03.bak` 是更早的版本，不含 #1123 對 §8 的補充）。理由：那五節是
> 行為矯正——教模型怎麼分派、怎麼摘要、怎麼升級，屬於較舊模型需要的補丁。保留下來的是
> **本環境的事實**（§0 有哪些 agent 型別、§3.5 subagent 不繼承 deny 清單、§7 並行時的實際限制）
> 與**使用者設定的判準**（§3 模型選擇、§5 驗收方式）。章節編號刻意不重排，方便對照 backup。
> 若某條被移除的紀律在實務上重複絆倒兩次以上，再單獨加回來——只錯一次不加。
